...
- **Memory**: Be mindful of WebSocket connections and implement proper error handling
...
<unifi_protect_specifics>
## UNIFI PROTECT SPECIFIC PATTERNS

### Authentication
- Use ProtectApiClient from uiprotect SDK
- Handle self-signed certificates gracefully
- Implement proper reconnection logic for WebSocket events

### Data Collection
- Bootstrap data provides initial system state
- WebSocket for real-time events (motion, camera status)
- Periodic API calls for statistics and health

### Camera Handling
- Cameras identified by unique ID (not MAC address)
- Camera models vary significantly in capabilities
- Smart detections (person, vehicle) only on certain models

### Motion Events
- Events arrive via WebSocket in real-time
- Events have types: motion, person, vehicle, package
- Duration calculated from start/end timestamps
- Zone information available for some cameras

### Recording Management
- Recording modes: always, motion, smart
- Storage calculated per camera
- Recording quality affects bandwidth and storage

### Common Pitfalls
- UniFi Protect API can be slow - use appropriate timeouts
- WebSocket can disconnect - implement reconnection
- Not all cameras support all features - check capabilities
- NVR can be on different subnet - handle network issues
</unifi_protect_specifics>